...
export function LapTimes({ laps }: LapTimesProps) {
  ...
  const checkScroll = () => {
    ...
    setShowTopScrim(isScrollable && scrollTop > 10);
    setShowBottomScrim(
    ...
    <div className="w-full">
      <div className="border rounded-[1rem] overflow-hidden bg-muted p-1 flex flex-col">
        ...
        <div className="relative">
          {showTopScrim && (
            <div className="absolute top-0 left-0 right-0 h-8 bg-gradient-to-b from-background to-transparent pointer-events-none z-10 rounded-t-[0.8rem]" />
          )}
          {showBottomScrim && (
            <div className="absolute bottom-0 left-0 right-0 h-8 bg-gradient-to-t from-background to-transparent pointer-events-none z-10 rounded-b-[0.8rem]" />
          )}
          <div
            ref={scrollRef}
            className="max-h-64 overflow-y-auto bg-background rounded-[0.8rem] w-full laps-list"
          >
            {laps.length === 0 ? (
